...
export function AddBudgetDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger
        className={`text-sm cursor-pointer font-semibold rounded-2xl text-white p-2 bg-primary hover:bg-primary/90 transition-all duration-300 ease-in-out flex justify-center ${className}`}
      >
        {triggerSentence}
      </DialogTrigger>
      <DialogContent>
